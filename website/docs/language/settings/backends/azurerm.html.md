...
* `key_vault_key_identifier` - (Optional) The identifier of the [Azure Key Vault key](https://www.terraform.io/docs/providers/azurerm/d/key_vault_key.html). If this value is provided AES-256 will be used to perform client-side encryption of the blob used by the Terraform's State file. The user or service principal should have the following access policies on Key Vault for `Keys`: `Get`, `List`, `Wrap` and `Unwrap`. This can also be sourced from the `ARM_KEY_VAULT_KEY_IDENTIFIER` environment variable.
